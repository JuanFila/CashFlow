...
using CashFlow.Domain.Extetnsions;
...
public class GenerateExpensesReportPdfUseCase : IGenerateExpensesReportPdfUseCase
{
    ...
    private Document CreateDocument(DateOnly month)
    {
        ...
        document.Info.Title = $"Relatório do mês {month:Y}";
    ...
    private void CreateTotalSpentSection(Section page, DateOnly month, decimal totalExpenses)
    {
        ...
        var title = string.Format("Valor total gasto no mês {0:Y}", month);

        paragraph.AddFormattedText(title, new Font { Name = FontHelper.RALEWAY_REGULAR, Size = 15 });

        paragraph.AddLineBreak();

        paragraph.AddFormattedText($"{totalExpenses} {CURRENCY_SYMBOL}", new Font { Name = FontHelper.WORKSANS_BOLD, Size = 50 });
    }
